...
# ✓ CORRECTED CALCULATIONS (EI: 1.7%, CPP: 5.7%, Vacation: 1.25%)
# ✓ DYNAMIC PERIOD SELECTION
# ✓ STAT HOLIDAYS MAPPING
# ✓ USER API KEY INPUT
# ✓ PERFECT YTD FORMULA WITH MAX LIMITS
# ✓ 2025/2026 CPP & EI THRESHOLD LOGIC
# """
...
#         3. Hourly × Hours = Regular
#         4. Hourly × 1.5 × OT = Overtime
#         5. Hourly × Vacation = Vacation Pay (1.25% of gross)
#         6. Hourly × Stat = Stat Holiday Pay
#         7. Gross = Regular + OT + Vacation + Stat
#         8. CPP = 5.7%, EI = 1.7%, Tax = 15%
#         9. Vacation = 1.25% of current gross
#         10. YTD = Period × Current + Extras (CPP +$133, EI +$23)
...
#         self.add_log("   ✅ CPP: 5.7% | EI: 1.7% | Tax: 15% | Vacation: 1.25%")
#         self.add_log("   ✅ YTD MAX: CPP ${:,.2f} | EI ${:,.2f}".format(
...
#         # STEP 1: CALCULATE
#         self.add_log("📊 STEP 1: Calculating (WITH YTD LIMITS)")
#         self.add_log("-" * 90)
...
#             self.add_log(f"   ⚠️  CPP YTD LIMIT REACHED: ${calcs['cpp_ytd']:,.2f} (Max: ${calcs['cpp_max']:,.2f})")
#         else:
#             self.add_log(f"   CPP YTD: ${calcs['cpp_ytd']:,.2f} / ${calcs['cpp_max']:,.2f}")
            
#         if calcs['ei_limit_hit']:
#             self.add_log(f"   ⚠️  EI YTD LIMIT REACHED: ${calcs['ei_ytd']:,.2f} (Max: ${calcs['ei_max']:,.2f})")
#         else:
#             self.add_log(f"   EI YTD: ${calcs['ei_ytd']:,.2f} / ${calcs['ei_max']:,.2f}")
        
#         self.add_log("   ✅ All calculations verified!")
#         self.add_log("")
        
#         # STEP 2: EXTRACT
#         self.add_log("📖 STEP 2: Advanced Extraction")
#         self.add_log("-" * 90)
...
#         # STEP 3: AI MAPPING
#         self.add_log("🧠 STEP 3: Revolutionary AI Detection")
#         self.add_log("-" * 90)
...
#         # STEP 5: REPLACE
#         self.add_log("✏️  STEP 5: Precision Replacement")
#         self.add_log("-" * 90)
...
#     # Header
#     st.markdown('<div class="main-header">🚀 Paystub Generator v5.2</div>', unsafe_allow_html=True)
#     st.markdown('<div class="sub-header">✅ Enhanced with YTD Limits • CPP & EI Max Thresholds • Vacation 1.25%</div>', unsafe_allow_html=True)
...
#         with st.spinner("🔄 Processing paystub with YTD limits..."):
#             try:
#                 def add_log(msg):
#                     st.session_state.logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")
...
#                 st.success("✅ Paystub generated successfully with YTD limits!")
#                 st.balloons()
...
"""
AI-Powered DOCX Paystub Generator - Streamlit Web Application v5.3
✓ FIXED: Gemini API rate limiting (SINGLE call optimization)
✓ FIXED: String format errors (robust JSON parsing)
✓ FIXED: Document corruption (safe cell handling)
✓ FIXED: Mapping failures (enhanced detection)
✓ CORRECTED CALCULATIONS (EI: 1.7%, CPP: 5.7%, Vacation: 1.25%)
✓ PERFECT YTD FORMULA WITH MAX LIMITS
"""
...
st.set_page_config(
    page_title="Paystub Generator v5.3",
    page_icon="🚀",
    layout="wide",
    initial_sidebar_state="expanded"
)
...
class PerfectCalculator:
    """Implements client's EXACT formula with YTD LIMITS"""
...
class OptimizedAI:
    """FIXED: Single API call to avoid rate limits"""
    ...
    def intelligent_mapping(self, extracted_data: Dict, calculations: Dict) -> List[Dict]:
        """OPTIMIZED: Single AI call with complete mapping"""
        
        self.add_log("   🧠 AI: Single optimized analysis...")
...
Return ONLY the JSON array, nothing else:"""
        ...
        try:
            ...
            if not isinstance(mappings, list):
                self.add_log(f"      ⚠️  AI returned non-list: {type(mappings)}")
                mappings = []
            
            mappings = [m for m in mappings if isinstance(m, dict) and m.get('old_value') and m.get('new_value')]
            
        except json.JSONDecodeError as e:
            self.add_log(f"      ⚠️  JSON parse error: {e}")
            self.add_log(f"      Raw response: {response.text[:200]}...")
            mappings = []
        except Exception as e:
            self.add_log(f"      ⚠️  AI call failed: {e}")
            mappings = []
...
class SafeReplacer:
    """FIXED: Safe document manipulation without corruption"""
    
    @staticmethod
    def find_and_replace_safe(doc: Document, mappings: List[Dict], 
                             extracted_data: Dict, add_log_callback=None) -> int:
        """Safe multi-strategy replacement"""
        ...
        for mapping in mappings:
            try:
                ...
                if location:
                    success = SafeReplacer._replace_by_location(
                        doc, location, old_val, new_val
                    )
                    if success:
                ...
                for val_info in extracted_data['all_values']:
                    if SafeReplacer._values_match(val_info['value'], old_val):
                        if val_info['type'] == 'table_cell' and 'cell' in val_info:
                            success = SafeReplacer._replace_in_cell_safe(
                                val_info['cell'], old_val, new_val
                            )
                            if success:
                                add_log(f"      ✅ {field}: '{old_val}' → '{new_val}'")
    ...
    def _replace_by_location(doc: Document, location: str, 
                            old_val: str, new_val: str) -> bool:
        """Replace at specific location safely"""
        match = re.match(r'T(\d+)R(\d+)C(\d+)', location)
        if not match:
            return False
        ...
        try:
            ...
            return SafeReplacer._replace_in_cell_safe(cell, old_val, new_val)
...
class Validator:
    ...
    def validate_all(calculations: Dict, mappings: List[Dict], 
        ...
        if not mappings:
            errors.append("No mappings created - AI may need better template")
        
        return len(errors) == 0, errors


class OptimizedSystem:
    """FIXED: Optimized system with single AI call"""
    ...
    def generate(self, input_doc: Document, yearly_income: float, period: int,
                 ...
                 vacation_hours: float = 0.0, stat_holidays: float = 0.0,
                 year: int = 2025) -> Tuple[Document, int, Dict]:
        """Generate paystub with optimized AI"""
        
        self.add_log("="*90)
        self.add_log(f"PAYSTUB GENERATOR v5.3 - FIXED VERSION ({year})")
        self.add_log("   ✅ Single AI call optimization")
        self.add_log("   ✅ Safe document handling")
        self.add_log("   ✅ Robust error handling")
        self.add_log("="*90)
        self.add_log("")
        ...
        self.add_log(f"   Period: {period} of 26 ({year})")
        self.add_log(f"   Gross: ${calcs['gross_current']:,.2f}")
        self.add_log(f"   Net: ${calcs['net_current']:,.2f}")
        ...
        self.add_log(f"   Values found: {len(extracted['all_values'])}")
        self.add_log("   ✅ Extracted")
        self.add_log("")
        
        self.add_log("🧠 STEP 3: AI Mapping (Single Call)")
        self.add_log("-" * 90)
